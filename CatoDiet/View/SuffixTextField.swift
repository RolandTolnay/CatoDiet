...
public class SuffixTextField: UITextField {
  ...
  required public init?(coder: NSCoder) {
    ...
    addTarget(self, action: #selector(textFieldDidChange(sender:)),
              for: .editingChanged)
    clipsToBounds = false
  }
  ...
  override public func draw(_ rect: CGRect) {
    ...
    if (suffix ?? "").isEmpty == false && (text ?? "").isEmpty == false {
      ...
      let rect = CGRect(x: suffixXPosition + spacing + offset,
                        y: top,
                        width: width,
                        height: height)
    ...
    if (prefix ?? "").isEmpty == false && (text ?? "").isEmpty == false {
      ...
      let offset = (width + spacing) / 2
      let rect = CGRect(x: prefixXPosition + offset, y: top,
